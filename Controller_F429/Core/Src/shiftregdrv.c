...
void switch_digit(uint8_t place)
{
	for(int i = 0; i < 8; i++)
	{
		HAL_GPIO_WritePin(GPIOF, DIG_CLK_Pin, 0);
		HAL_GPIO_WritePin(GPIOF, DIG_DATA_Pin, 0);
		HAL_GPIO_WritePin(GPIOF, DIG_DATA_Pin, ((digit_places[place] >> i) & 1));
		HAL_GPIO_WritePin(GPIOF, DIG_CLK_Pin, 1);
		HAL_GPIO_WritePin(GPIOF, DIG_CLK_Pin, 0);
	}
	HAL_GPIO_WritePin(GPIOF, DIG_CLK_Pin, 0);
	HAL_GPIO_WritePin(GPIOF, DIG_DATA_Pin, 0);
...
void print_digit(uint8_t place, uint8_t point)
{
	for(int i = 0; i < 8; i++)
	{
		HAL_GPIO_WritePin(GPIOF, SEG_CLK_Pin, 0);
		HAL_GPIO_WritePin(GPIOF, SEG_DATA_Pin, 0);
		if(point == 1){
			HAL_GPIO_WritePin(GPIOF, SEG_DATA_Pin, (((symbols[place] | symbols[10]) >> i) & 1));
		}
		else{
			HAL_GPIO_WritePin(GPIOF, SEG_DATA_Pin, ((symbols[place] >> i) & 1));
		}
		HAL_GPIO_WritePin(GPIOF, SEG_CLK_Pin, 1);
		HAL_GPIO_WritePin(GPIOF, SEG_CLK_Pin, 0);
	}
	HAL_GPIO_WritePin(GPIOF, SEG_CLK_Pin, 0);
	HAL_GPIO_WritePin(GPIOF, SEG_DATA_Pin, 0);
...
void latch_out()
{
	HAL_GPIO_WritePin(GPIOF, COM_LATCH_Pin, 0);
	HAL_GPIO_WritePin(GPIOF, COM_LATCH_Pin, 1);
	HAL_GPIO_WritePin(GPIOF, COM_LATCH_Pin, 0);
}
